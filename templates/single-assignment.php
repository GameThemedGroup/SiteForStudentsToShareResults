...
<?php
  ...
  if($isStudent)
  {
    $isEnrolled = true; // TODO: replace with function to check if student is enrolled
  }
...
<?php endif ?>
      ...
      <div id="assignment-buttons">
        <div id="assignment-button">
          <?php if($view == 'description') : ?><b>Description</b>
          <?php else : ?><a href="<?php echo site_url('/assignment/?assignid=' . $assignmentId . '&view=description') ?>">Description</a>
          <?php endif ?>
        </div>
        <div id="assignment-button">
          <?php if($view == 'applet') : ?><b>Applet</b>
          <?php else : ?><a href="<?php echo site_url('/assignment/?assignid=' . $assignmentId . '&view=applet') ?>">Applet</a>
          <?php endif ?>
        </div>
      </div>
    </div>
    <div id="assignment-description">
      <?php if($view == 'description') : ?><?php echo nl2br($assignment->post_content); ?>
...
<?php if($isEnrolled && $status) : ?>
        ...
        <tr>
          <th class="action" colspan="3">
            <b><a class="action" href="<?php echo site_url('/manage-assignments/?courseid=' . $courseId . '&assignid=' . $assignmentId) ?>">Submit Assignment</a></b>
          </th>
        </tr>
<?php endif; ?>
      </tbody>
